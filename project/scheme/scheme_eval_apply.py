...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        operands = rest.map(lambda r: scheme_eval(r, env))
...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 6
    new_e = expressions.map(lambda e: scheme_eval(e, env))
    result = None
    while new_e is not nil:
        result = new_e.first
        new_e = new_e.rest
    return result
    # END PROBLEM 6
